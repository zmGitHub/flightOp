    ...
    template: Ember.Handlebars.compile(
        '{{#unless view.isActive}}' +
          '{{#if view.content.model}}' +
            '{{#if view.content.icon}}' +
              '<i {{bind-attr class=view.content.icon}}></i>' +
            '{{/if}}' +
    ...
    actions: {
      ...
      updateCrumbsByRoute: function() {
        ...
        routes.get('router.currentHandlerInfos').forEach(function(route, i, arr) {
          var crumb, displayName, name, routeName, displayIcon, _ref, _ref1, _ref2;
          name = route.name;
          ...
          //获取按钮图标
          displayIcon = "icon-bookmark-empty";

          if ((_ref1 = route.handler.breadcrumbs) != null ? _ref1.icon : void 0) {
            displayIcon = route.handler.breadcrumbs.icon;
          }

          if(route.name === 'application')
            displayIcon = 'icon-home';
